...
class Model extends \Kotchasan\Model
{
    ...
    public static function toDataTable($params)
    {
        $where = [];
        if ($params['sender'] > 0) {
            $where[] = ['A.sender_id', $params['sender']];
        }
        if ($params['urgency'] > -1) {
            $where[] = ['A.urgency', $params['urgency']];
        }
        return static::createQuery()
            ...
            ->from('edocument A')
            ->join('edocument_download D', 'LEFT', ['D.document_id', 'A.id'])
            ->where($where)
            ->groupBy('A.id');
    }
            ...
            if (Login::notDemoMode($login)) {
                ...
                if (preg_match_all('/,?([0-9]+),?/', $request->post('id')->filter('0-9,'), $match)) {
                    if ($action === 'delete' && Login::checkPermission($login, 'can_upload_edocument')) {
                        ...
                        // ลบข้อมูล
                        $this->db()->createQuery()
                            ->delete('edocument', ['id', $match[1]])
                            ->execute();
                        $this->db()->createQuery()
                            ->delete('edocument_download', ['document_id', $match[1]])
                            ->execute();
                        // Log
