    ...
    public static function get($id)
    {
        if (empty($id)) {
            ...
            $q1 = static::createQuery()
                ...
                ->join('inventory_user U', 'LEFT', ['U.stock_id', 'S.id'])
                ->where([
                    ['S.inventory_id', $id],
            ...
            $query = static::createQuery()
                ->from('inventory R')
                ->join([$q1, 'S'], 'LEFT', ['S.inventory_id', 'R.id'])
            ...
            foreach (Language::get('INVENTORY_CATEGORIES', []) as $key => $label) {
                $query->join('inventory_meta M'.$n, 'LEFT', [['M'.$n.'.inventory_id', 'R.id'], ['M'.$n.'.name', $key]]);
                ...
                try {
                    ...
                    if ($index) {
                        ...
                        if ($stock['product_no'] == '') {
                            ...
                            $search = $db->first($table_stock, ['product_no', $stock['product_no']]);
                            if ($search && ($index->stock_id == 0 || $index->stock_id != $search->id)) {
                                $ret['ret_product_no'] = Language::replace('This :name already exist', [':name' => Language::get('Serial/Registration No.')]);
                            }
                        }
                        if ($save['topic'] == '') {
                            // ไม่ได้กรอก topic
