                    ...
                    if ($index && ($index->id == 0 || $login['id'] == $index->customer_id || $can_manage_repair)) {
                        ...
                        if (empty($ret)) {
                            ...
                            if ($can_manage_repair && $index->id > 0) {
                                // สามารถจัดการรายการซ่อมได้
                                $ret['location'] = $request->getUri()->postBack('index.php', ['module' => 'repair-setup', 'id' => null]);
                            } else {
                                // ใหม่
                                $ret['location'] = $request->getUri()->postBack('index.php', ['module' => 'repair-history', 'id' => null]);
                            }
                            // clear
                            $request->removeToken();
                        }
