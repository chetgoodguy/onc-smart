...
class Model extends \Kotchasan\Model
{
    ...
    public static function send($order)
    {
        ...
        if (self::$cfg->demo_mode) {
            ...
            $where = [
                // ผู้ทำรายการ, คนขับรถ
                ['U.id', [$order['member_id'], $order['chauffeur']]],
                // แอดมิน
                ['U.status', 1]
            ];
    ...
    private static function vehicle($vehicle_id, $chauffeur)
    {
        ...
        $n = 1;
        foreach (Language::get('CAR_SELECT', []) as $key => $label) {
            $query->join('vehicles_meta M'.$n, 'LEFT', [['M'.$n.'.vehicle_id', 'V.id'], ['M'.$n.'.name', $key]]);
            $query->join('category C'.$n, 'LEFT', [['C'.$n.'.type', $key], ['C'.$n.'.category_id', 'M'.$n.'.value']]);
