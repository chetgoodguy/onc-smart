...
function initLineGroup() {
  $G('line_department').Ready(function() {
    var doChanged = function() {
      $E('line_name').parentNode.parentNode.className = $E('line_department').value == 0 ? 'item' : 'hidden';
    };
    $G('line_department').addEvent('change', doChanged);
    doChanged.call(this);
  });
}
