    ...
    public static function get($id, $login)
    {
        if (empty($id)) {
            ...
            $result = static::createQuery()
                ->from('edocument E')
                ->join('edocument_download D', 'LEFT', [['D.document_id', 'E.id'], ['D.department_id', '']])
                ...
                try {
                    ...
                    if ($index && ($index->id == 0 || $login['id'] == $index->sender_id || Login::checkPermission($login, 'can_upload_edocument'))) {
                        ...
                        if ($save['document_no'] == '') {
                            ...
                            $search = $db->first($table_edocument, ['document_no', $save['document_no']]);
                        ...
                        if (empty($ret)) {
                            ...
                            // query ผู้รับเดิม
                            foreach ($this->db()->select($table_download, ['document_id', $save['id']]) as $item) {
                            ...
                            // ลบ ผู้รับเดิม
                            $this->db()->delete($table_download, ['document_id', $save['id']], 0);
                            ...
                            $ret['location'] = $request->getUri()->postBack('index.php', ['module' => 'edocument-sent']);
                        }
                    }
