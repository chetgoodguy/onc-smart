    ...
    public static function get($id)
    {
        if (!empty($id)) {
            $query = static::createQuery()
                ->from('user U')
                ->join('user_meta M', 'LEFT', [['M.member_id', 'U.id'], ['M.name', 'department']])
                ->join('user_meta P', 'LEFT', [['P.member_id', 'U.id'], ['P.name', 'position']])
                ->where([
                    ['U.id', $id],
                    ['U.active', 1]
                ]);
            $select = ['U.*', 'M.value department', 'P.value position'];
            $n = 1;
            foreach (Language::get('PERSONNEL_DETAILS', []) as $key => $label) {
                $query->join('user_meta M'.$n, 'LEFT', [['M'.$n.'.member_id', 'U.id'], ['M'.$n.'.name', $key]]);
                    ...
                    if ($login && $login['active'] == 1) {
                        ...
                        if ($user['name'] == '') {
                            ...
                            foreach ($request->getUploadedFiles() as $item => $file) {
                                ...
                                if ($file->hasUploadFile()) {
                                    if (!File::makeDirectory($dir)) {
                                        ...
                                        try {
                                            $file->cropImage(self::$cfg->img_typies, $dir.$index->id.self::$cfg->stored_img_type, self::$cfg->personnel_w, self::$cfg->personnel_h);
                                        } catch (\Exception $exc) {
                                            // ไม่สามารถอัปโหลดได้
                            ...
                            if (empty($ret)) {
                                ...
                                $this->db()->delete($table, ['member_id', $index->id], 0);
