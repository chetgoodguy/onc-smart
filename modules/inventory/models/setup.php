...
class Model extends \Kotchasan\Model
{
    ...
    public static function toDataTable()
    {
        ...
        $q2 = static::createQuery()
            ...
            ->join([$q1, 'N'], 'INNER', [['N.stock_id', 'I.stock_id'], ['N.create_date', 'I.create_date']]);
        $select = [
            'R.id',
            'S.product_no',
            'R.topic',
        ...
        $query = static::createQuery()
            ->from('inventory R')
            ->join('inventory_stock S', 'LEFT', ['S.inventory_id', 'R.id'])
            ->join([$q2, 'I'], 'LEFT', ['I.stock_id', 'S.id'])
            ...
            if (Login::notDemoMode($login) && Login::checkPermission($login, 'can_manage_inventory')) {
                ...
                if (preg_match_all('/,?([0-9]+),?/', $request->post('id')->filter('0-9,'), $match)) {
                    if ($action === 'delete') {
                        ...
                        $stock = [];
                        foreach ($db->select($table_stock, ['inventory_id', $match[1]]) as $item) {
                            $stock[] = $item['id'];
                        }
                        $db->delete($table, ['id', $match[1]], 0);
                        $db->delete($this->getTableName('inventory_meta'), ['inventory_id', $match[1]], 0);
                        $db->delete($this->getTableName('inventory_user'), ['stock_id', $stock], 0);
                        $db->delete($table_stock, ['inventory_id', $match[1]], 0);
                        // ลบรูปภาพ
                        ...
                        if ($search) {
                            ...
                            $db->update($table, $search->id, ['status' => $status]);
                            // คืนค่า
