...
class Model extends \Kotchasan\Model
{
    ...
    public static function getAllStatus($id)
    {
        return static::createQuery()
            ...
            ->join('user U', 'LEFT', ['U.id', 'S.operator_id'])
        ...
        if ($request->initSession() && $request->isReferer() && $login = Login::isMember()) {
            if (Login::notDemoMode($login)) {
                ...
                if (preg_match('/^delete_([0-9a-z]+)$/', $id, $match)) {
                    ...
                    if ($action === 'delete' && Login::checkPermission($login, ['can_manage_repair', 'can_repair'])) {
                        // ลบรายละเอียดซ่อม
                        $this->db()->delete($this->getTableName('repair_status'), ['id', (int) $match[1][0]]);
                        // log
